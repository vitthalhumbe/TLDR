  ...
  if (submitted) {
    return (
      <FadeIn delay={100} className="max-w-3xl mx-auto text-center py-12">
        ...
        <div className="inline-flex items-center justify-center w-40 h-40 rounded-full border-8 border-gray-100 mb-8 relative">
        ...
        <div className="space-y-6 text-left">
          {quizzes.map((q, i) => {
            const sel = answers[i] || [];
            const isCorrect = q.type === "mcq" ? sel[0] === q.answer : [...(q.answer)].sort().join() === [...sel].sort().join();
            return (
                ...
                <div className="flex gap-4 items-start mb-4">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isCorrect ? "bg-green-500" : "bg-red-500"}`}>
                    <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      {isCorrect ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12" />}
                    </svg>
                  </div>
                  <div>
    ...
    <FadeIn delay={100} className="max-w-3xl mx-auto space-y-8">
      {quizzes.map((q, i) => (
        <div key={i} className="bg-white border border-gray-200 p-8 md:p-10 rounded-[2rem] shadow-sm">
          <div className="mb-6">
            <span className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3 block">Question {i + 1}</span>
            <p className="text-xl font-bold text-gray-900 leading-snug">{q.question}</p>
          </div>
          <div className="space-y-3">
            {q.options.map(opt => {
              ...
              return (
                <label key={opt} className={`flex items-center gap-4 p-5 rounded-2xl border-2 cursor-pointer transition-all ${isSelected ? "border-black bg-gray-50" : "border-gray-100 hover:border-gray-200 hover:bg-gray-50/50"}`}>
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${isSelected ? "border-black bg-black" : "border-gray-300"}`}>
